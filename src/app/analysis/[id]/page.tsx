    ...
    <div className="mx-auto max-w-6xl px-5 py-10 md:py-14">
      ...
      <div className="mt-8 grid lg:grid-cols-12 gap-6">
        <section className="lg:col-span-7 space-y-6">
          <UpsideCard
            offer={a.insurerOffer}
            fair={a.estimatedFairValue}
            upside={a.upside}
          />
          <FindingsCard />
          <ComparablesCard />
...
function UpsideCard({
  ...
  const pct = Math.round((offer / fair) * 100);
  return (
        ...
        <div>
          ...
          <div className="mt-3 h-3 rounded-full bg-white/15 overflow-hidden">
            <div
              ...
              style={{ width: `${pct}%` }}
              aria-label={`Offer is ${pct}% of fair value`}
            />
          </div>
          <div className="mt-2 text-xs text-shield-100">
            Current offer is <span className="font-semibold text-white">{pct}%</span>{" "}
            of independently estimated value.
          </div>
        </div>
      </div>
    </div>
...
function FindingIcon({ kind }: { kind: string }) {
  const map: Record<string, { bg: string; fg: string; path: string }> = {
    ...
    regulation: {
      bg: "bg-accent/10",
      fg: "text-accent-dark",
      path: "M12 2l9 4v6c0 5-3.5 8.5-9 10-5.5-1.5-9-5-9-10V6l9-4z",
    },
    valuation: {
      bg: "bg-amber-50",
      fg: "text-warn",
    ...
    procedure: {
      bg: "bg-shield-50",
      fg: "text-shield-700",
      path: "M12 8v4l3 2M12 22a10 10 0 1 1 0-20 10 10 0 0 1 0 20z",
    },
  };
  const m = map[kind] || map.exclusion;
  return (
    <span
      className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${m.bg} ${m.fg}`}
    >
...
function ComparablesCard() {
  const rows = [
    { ref: "AU-MTR-08172", insurer: "Auric Mutual", outcome: "$7,150", note: "Same exclusion contested" },
    { ref: "AU-MTR-04429", insurer: "Cobalt General", outcome: "$6,980", note: "GICOP §8.7 cited" },
    { ref: "AU-MTR-11203", insurer: "Auric Mutual", outcome: "$7,640", note: "Endorsement E-12 invoked" },
  ];
  return (
    <Card eyebrow="Comparables" title="Similar claims, similar outcomes">
      <div className="-mx-6 -mb-6 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.ref} className="border-t border-black/5">
                <td className="px-6 py-3 font-mono text-xs text-ink-soft">{r.ref}</td>
                <td className="px-6 py-3">{r.insurer}</td>
                <td className="px-6 py-3 font-medium">{r.outcome}</td>
  ...
  return (
    <Card eyebrow="Play-by-play" title="Your next moves">
      <ol className="space-y-4">
        {mockAnalysis.nextSteps.map((s, i) => (
          <li key={i} className="flex gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-[11px] rounded-full bg-canvas px-2 py-0.5 text-ink-muted">
                  {s.due}
                </span>
              </div>
...
function ScoreCard({ score, label }: { score: number; label: string }) {
  const pct = score;
  const dash = (pct / 100) * 2 * Math.PI * 42;
      ...
      <div className="flex items-center gap-5">
        <div className="relative h-28 w-28">
          <svg viewBox="0 0 100 100" className="h-28 w-28 -rotate-90">
            ...
            <circle
              ...
              fill="none"
              strokeLinecap="round"
              strokeDasharray={`${dash} ${full - dash}`}
            />
          </svg>
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-ink-muted leading-relaxed">
            Based on policy fit, regulatory leverage, valuation gap, and
            outcomes of 41 comparable claims.
          </p>
        </div>
      </div>
    </Card>
  ...
  return (
    <Card eyebrow="Ready to send" title="Generated documents">
      <ul className="space-y-2">
        {mockAnalysis.generatedDocs.map((d) => (
          <li
            key={d.name}
            className="flex items-center justify-between rounded-xl border border-black/5 bg-canvas/60 px-4 py-3"
          >
            ...
            <div className="flex gap-2">
              <button className="text-xs font-medium px-3 py-1.5 rounded-full bg-white border border-black/10 hover:border-black/30">
                Preview
              </button>
              <button className="text-xs font-medium px-3 py-1.5 rounded-full bg-shield-600 text-white hover:bg-shield-700">
                Download
              </button>
            </div>
          </li>
  ...
  return (
    <Card eyebrow="Preview" title="Your appeal letter">
      <pre className="whitespace-pre-wrap text-[13px] leading-relaxed font-sans text-ink-soft bg-canvas/60 rounded-xl border border-black/5 p-4 max-h-80 overflow-auto">
        {mockAnalysis.appealLetter}
      </pre>
      <div className="mt-3 flex flex-wrap gap-2">
        <button className="rounded-full bg-shield-600 text-white text-sm font-medium px-4 py-2 hover:bg-shield-700">
          Send via ClaimShield
        </button>
        <button className="rounded-full bg-white border border-black/10 text-sm font-medium px-4 py-2 hover:border-black/30">
          Copy text
        </button>
        <button className="rounded-full bg-white border border-black/10 text-sm font-medium px-4 py-2 hover:border-black/30">
          Download .docx
...
function DisclaimerCard() {
  return (
    <div className="rounded-2xl border border-black/5 bg-white/60 p-5 text-xs text-ink-muted leading-relaxed">
      ...
      <p className="mt-2">
        ClaimShield helps you understand your policy and prepare documents.
        It does not provide legal advice and is not a law firm. For complex
        matters we&apos;ll flag when a licensed professional makes sense.
      </p>
    </div>
  );
}
